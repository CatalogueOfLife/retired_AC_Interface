<?php
	...
	if ($id !== "" && $error == "") {
		...
		$query = "SELECT `name_code` FROM `scientific_names` WHERE `record_id` = '$id' " ;
	...
	if ($search_string . $unique_identifier !== "" && $error == "") {
		require_once "../connect_to_database.php" ;
		...
		if ($unique_identifier !== "") {
			$query .= "WHERE `name_code` = '$unique_identifier' " ;
		} else {
	...
	for ($i = 0 ; $i < $number_of_results ; $i++) {
		...
		if ($name_status === "common name") {
			...
			$result2 = mysql_query($query2) ;
			if (mysql_error() != "") {
				return errorMessage("Database query failed") ;
			}
			...
			$query2 = "SELECT `name_with_italics` AS name_html, 
						   `taxon` as rank
					 FROM `taxa` WHERE `record_id` = '$record_id'" ;
			$result2 = mysql_query($query2) ;
			if (mysql_error() != "") {
				return errorMessage("Database query failed") ;
			}
			...
			if ($rank == "Species" || $rank === "Infraspecies") {
				$query2 = "SELECT `scientific_names`.`genus` AS genus, 
							  ...
							  `scientific_names`.`record_id` as record_id,
		...
		if (strpos($results["names"][$i]["name_status"],"accepted name") !== false && $name_status !== "common name" ) {
			...
			$query2 = "SELECT `scientific_names`.`record_id` AS record_id, 
							 `scientific_names`.`web_site` AS this_link, 
			...
			if ($row2 = mysql_fetch_array($result2)) {
				...
				return errorMessage("Database query failed: accepted name not found") ;
			}
		...
		$parent_id = "" ;
		$query2 = "SELECT `parent_id` FROM `taxa` WHERE `name_code` = '$unique_identifier'" ;
		$result2 = mysql_query($query2) ;
		if (mysql_error() != "") {
			return errorMessage("Database query failed") ;
		}
		...
		if ($response == "full") {
			...
			if ($rank == "Species") {
				$accepted_name_array["infraspecies_for_this_species"] = array() ;
				...
				while ($row2 = mysql_fetch_row($result2)) {
					$this_id = $row2[0] ;
					$this_infraspecies_marker = $row2[1] ;
					$this_infraspecies = $row2[2] ;
					$this_author = $row2[3] ;
					$this_name_code = $row2[4] ;
					$this_accepted_name_code = $row2[5] ;
					if ($this_accepted_name_code == "") {
						$this_accepted_name_code = $this_name_code ;
					}
					if ($this_accepted_name_code == $this_name_code) {
						$infraspecies_results = array() ;
						$infraspecies_results["id"] = $this_id ;
						$infraspecies_results["name"] = compileScientificName($genus,$species,$this_infraspecies_marker,$this_infraspecies,$this_author,$this_kingdom, "no") ;
						$infraspecies_results["name_html"] = compileScientificName($genus,$species,$this_infraspecies_marker,$this_infraspecies,$this_author,$this_kingdom, "yes") ;
						$infraspecies_results["genus"] = $genus ;
						$infraspecies_results["species"] = $species ;
						$infraspecies_results["infraspecies_marker"] = $this_infraspecies_marker ;
						$infraspecies_results["infraspecies"] = $this_infraspecies ;
						$infraspecies_results["author"] = $this_author ;
						$infraspecies_results["url"] = $path . "show_species_details.php?record_id=" . $this_id ;
						array_push($accepted_name_array["infraspecies_for_this_species"] , $infraspecies_results) ;
					}
				}
			}
			...
			$accepted_name_array["synonyms"] = array() ;
			if ($number_of_synonyms > 0) {
				for ($j = 0 ; $j < $number_of_synonyms ; $j++) {
					$accepted_name_array["synonyms"][$j] = array() ;
					...
					$accepted_name_array["synonyms"][$j]["id"] = $record_id ;
					$accepted_name_array["synonyms"][$j]["name"] = $name ;
					$accepted_name_array["synonyms"][$j]["rank"] = $rank ;
					$accepted_name_array["synonyms"][$j]["name_status"] = $name_status ;
					$accepted_name_array["synonyms"][$j]["name_html"] = $name_html ;
					$accepted_name_array["synonyms"][$j]["genus"] = $genus ;
					$accepted_name_array["synonyms"][$j]["species"] = $species ;
					if ($rank !== "Species") {
						$accepted_name_array["synonyms"][$j]["infraspecies_marker"] = $infraspecies_marker ;
						$accepted_name_array["synonyms"][$j]["infraspecies"] = $infraspecies ;
					}
					$accepted_name_array["synonyms"][$j]["author"] = $author ;
					$accepted_name_array["synonyms"][$j]["additional_data"] = $additional_data ;
					$accepted_name_array["synonyms"][$j]["url"] = $URL ;
					$accepted_name_array["synonyms"][$j]["online_resource"] = $this_link ;
					$accepted_name_array["synonyms"][$j]["source_database"] = $source_database ;
					$accepted_name_array["synonyms"][$j]["source_database_url"] = $source_database_url ;
					$accepted_name_array["synonyms"][$j]["record_scrutiny_date"] = $record_scrutiny_date ;
					$references = getReferences($synonym_unique_identifier) ;
					if ($references === false) {
						return errorMessage("Database query failed") ;
					}
					$accepted_name_array["synonyms"][$j]["references"] = $references ;
				}
			}
			...
			$accepted_name_array["common_names"] = array() ;
			if ($number_of_common_names > 0) {
				for ($j = 0 ; $j < $number_of_common_names ; $j++) {
					$accepted_name_array["common_names"][$j] = array() ;
					$row2 = mysql_fetch_array($result2) ;
					foreach ($row2 as $key => $value) {
						$$key = $value;
					}
					$common_name = $row2[0] ;
					$language = $row2[1] ;
					$accepted_name_array["common_names"][$j]["name"] = $common_name ;
					$accepted_name_array["common_names"][$j]["language"] = $language ;
					$accepted_name_array["common_names"][$j]["country"] = $country ;
					$references = getCommonNameReferences($common_name, $language, $country, $unique_identifier) ;
					if ($references === false) {
						return errorMessage("Database query failed") ;
					}
					$accepted_name_array["common_names"][$j]["references"] = $references ;
				}
			}
		}
	...
	function getDistribution($name_code) {
		$distribution = "" ;
		$query = "SELECT `distribution` FROM `distribution` WHERE `name_code` = '$name_code' " ;
		$result = mysql_query($query);
		if (mysql_error() != "") {
			return false ;
		}
		...
		if ($format == "php") {
			...
			foreach ($results["names"] as $name_record) {
				...
				if (array_key_exists("classification", $name_record)) {
					...
					$properties = array("name","rank","name_html","url") ;
				...
				if (array_key_exists("infraspecies_for_this_species", $name_record)) {
					$output .= "<infraspecies_for_this_species>\n" ;
					$properties = array("id","name","name_html","genus","species","infraspecies_marker","infraspecies","url") ;
					foreach ($name_record["infraspecies_for_this_species"] as $this_item) {
						$output .= "<infraspecies>\n" ;
						$output .= writeXMLPropertyValues($this_item,$properties,false) ;
						$output .= "</infraspecies>\n" ;
					}
