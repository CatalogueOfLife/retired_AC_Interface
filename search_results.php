...
<html>
<head>
<title>Catalogue of Life : 2006 Annual Checklist : Search results</title>
...
<?php
	...
	if ($error == "") {
		...
		if ($select != "" ) {
			if ($search_string == "") {
				$search_query = "SELECT $select FROM $from WHERE $where ORDER BY $order_by" ;
			} else {
...
//echo $search_query ;
		...
		if (!isset($_REQUEST["number_of_records_found"])) {
			...
			if ($query_number_of_hits != "") {
				$result = mysql_query($query_number_of_hits) or die("Error: MySQL query failed");
		...
		if ($number_of_records_found == "" && $query_number_of_hits == "") {
			$result = mysql_query($search_query) or die("Error: MySQL query failed");
		...
		} else if ($number_of_records_found > 0) {
