...
class Eti_View_Helper_GoogleAnalytics
{
     ...
     */
    protected static $_trackerOptionsByIds = array();
    
    /**
     * Available Trackers options
     */
    protected static $_availableOptions = array(// Standard Options
        'trackPageview', 'setVar',
        // ECommerce Options
        'addItem', 'addTrans', 'trackTrans',
        // Tracking Options
        'setClientInfo', 'setAllowHash', 'setDetectFlash', 'setDetectTitle',
        'setSessionTimeOut', 'setCookieTimeOut', 'setDomainName',
        'setAllowLinker', 'setAllowAnchor',
        // Campaign Options
    ...
    public function toString()
    {
        ...
        $xhtml[] = 'var gaJsHost = (("https:" == document.location.protocol) ' .
            '? "https://ssl." : "http://www.");';
        $xhtml[] = 'document.write(unescape("%3Cscript src=\'" + gaJsHost + ' .
        ...
        foreach (self::$_trackerOptionsByIds as $trackerId => $options) {
            ...
            // init tracker
            $xhtml[] = 'var ' . $trackerInstance . ' = ' .
                '_gat._getTracker("' . $trackerId . '");';
            // add options
            foreach ($options as $optionsData) {
                ...
                foreach ($optionsData as $arg) {
                    $optionArgs[] = is_numeric($arg) ? $arg : '"' .
                        addslashes($arg) . '"';
                }
                // add options
                $xhtml[] = $trackerInstance . '.' . $optionName . '(' .
                    implode(',', $optionArgs) . ');';
            }
            $i ++;
        }
