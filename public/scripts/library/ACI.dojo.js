...
dojo.addOnLoad(function(){
    dojo.declare('ACI.dojo.QueryStoreModel', dijit.tree.ForestStoreModel, {
		getChildren: function(parentItem, complete_cb, error_cb) {
			...
			this.store.fetch({ query: { id: parentId }, 
				onComplete: complete_cb, onError: error_cb });
		},
	...
	dojo.declare('ACI.dojo._TxTreeNode', dijit._TreeNode, {
		setLabelNode: function(label) {
			if(this.item.root || 
				this.tree.model.store.getValue(this.item, 'parentId') == 0) {			    
				return this.inherited(arguments);
		    }
			if(this.tree.model.store.getValue(this.item, 'numChildren') > 0) {
				var span = dojo.doc.createElement('span');				
				span.appendChild(
					dojo.doc.createTextNode(
						this.tree.model.store.getValue(this.item, 'type')
					)
				);
